...
class Block {
    ...
    constructor(index: number, hash: string, previousHash: string,
        ...
        this.nonce = nonce;
        // added marklet root hash in the block object
...
const calculateTransactionHash = (tx: Transaction) => {
    const concatedData = Object.keys(tx).map((key) => {
        if (key === 'txIns') {
            if (tx.txIns.length === 0) {
                return "";
            }
            else {
                tx.txIns.map(txIn => txIn.signature + txIn.txOutId + txIn.txOutIndex)
            ...
            if (tx.txOuts.length === 0) {
                ...
                tx.txOuts.map(txOut => txOut.address + txOut.amount)
                .reduce((acc, value) => value + acc);
            }
        }
    ...
    return CryptoJS.SHA256(CryptoJS.SHA256(concatedData)).toString();
}

// calculate merkle root hash in each block
const calculateMerkleRoot = (blockData: Transaction[]) => {
    const concatedString = blockData.map((tx) => calculateTransactionHash(tx)).reduce((acc, value) => value +acc);
    
    return CryptoJS.SHA256(CryptoJS.SHA256(concatedString)).toString();
}
...
const generateRawNextBlock = (blockData: Transaction[]) => {
    ...
    const merkleRoot: string = calculateMerkleRoot(blockData);
...
const findBlock = (index: number, previousHash: string, timestamp: number, 
                    data: Transaction[], difficulty: number, merkleRoot: string): Block => {
    let nonce = 0;
...
// define how many leading zeros to determine the difficulty
